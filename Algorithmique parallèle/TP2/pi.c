...
int main(int argc, char **argv)
{
  ...
  if (num != 0)
  {
    MPI_Ssend(&pi_part, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
  }
  ...
  if (num == 0)
  {
    ...
    for (int i = 1; i < nbP; ++i)
    {
      MPI_Recv(&message, 1, MPI_DOUBLE, num + i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      pi_part += message;
    }
  }
...
double integration(long n, double deb, double fin)
{
  ...
  for (i = 1; i < n; i++)
  { // Contributions des points internes de l'intervalle
    x = deb + h * i;
    somme += cercle(x);
  }
