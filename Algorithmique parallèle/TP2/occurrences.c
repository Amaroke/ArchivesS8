...
int main(int argc, char **argv)
{
  ...
  int taille = 1000000;         // Taille du texte à traiter
...
void compte_occurrences_mpi(int numP, int nbP, int tBloc, char *texte, int taille, int *occus)
{
  ...
  { // Gestion des travailleurs
    ...
    while (fini == 'N')
    {
      ...
      MPI_Recv(tampon, tBloc, MPI_CHAR, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &etat);

      // Récupération de la taille du message
      MPI_Get_count(&etat, MPI_CHAR, &nbRecus);

      // Distinction entre message de texte et message de fin (vide)
      if (nbRecus == 0)
      { // Taille nulle indique la fin du travail

        // Arrêt du travail
        fini = 'O';
      }
      else
      { // Taille non nulle indique un morceau de texte à traiter

        // Réception du texte
        MPI_Recv(tampon, nbRecus, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &etat);
